...
class Specification:
    ...
    def get_security_schemes(self, path: str, verb: str) -> List[dict]:
        return self.raw["paths"][path][verb]["security"]
...
def _walk_path(spec, *path: str, default=MISSING) -> Any:
    for segment in path:
...
def flatten_spec(spec: dict) -> dict:
    def resolve(node: _T) -> _T:
        ...
        return _walk_path(spec, *ref.split("/"))
